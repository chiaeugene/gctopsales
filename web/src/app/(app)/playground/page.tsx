...
export default function PlaygroundPage() {
  ...
  useEffect(() => {
    // Boss-only probe: the demo-chats API rejects agents, and a rejected probe
    // simply means no button. Cheaper than shipping role state to the client.
    fetch("/api/admin/demo-chats").then((res) => setCanDemo(res.ok)).catch(() => {});
  }, []);
  ...
  const chatList = (
    <div className="flex flex-col h-full min-h-0">
      <div className="p-3 border-b border-black/[0.06] space-y-2">
        ...
        {canDemo && (
        <>
        {/* Four real conversations to look at, generated by the live engine.
            Boss-only: agents' chat lists stay clean of demo content. */}
        <button
          onClick={buildDemoChats}
          disabled={demoBusy !== null || busy}
          title="Skincare, hair fall, bloating and stress in Mandarin. Real replies, takes a minute or two."
          className="w-full rounded-xl border border-black/10 px-3 py-2 text-[11px] font-medium text-black/55 transition-colors hover:border-[var(--accent)] hover:text-[var(--accent-ink)] disabled:opacity-40"
        >
