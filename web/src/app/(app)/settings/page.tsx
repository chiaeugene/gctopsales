...
export default function SettingsPage() {
  ...
  if (!settings) return <div className="text-sm text-neutral-400">{error || "Loading…"}</div>;
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="rounded-xl border border-amber-300 bg-amber-50 p-5 space-y-3">
        <h2 className="font-semibold text-amber-900">⚠️ Auto-confirm payments (high risk, off by default)</h2>
        ...
        <label className="flex items-center gap-2 text-sm font-medium text-amber-900">
          ...
          {savedCard === "autoconfirm" && <span className="text-emerald-700 text-xs">Saved ✓</span>}
        </label>
      </div>
    </div>
  );
  ...
  return (
    <div className="rounded-xl bg-white border border-neutral-200 p-5 space-y-3">
      ...
      <div className="space-y-2">
        {TONE_OPTIONS.map((o) => (
          <label key={o.value} className={"flex items-start gap-3 rounded-lg border p-3 cursor-pointer " + (tone === o.value ? "border-violet-500 bg-violet-50" : "border-neutral-200 hover:bg-neutral-50")}>
      ...
      <button onClick={() => props.onSave(tone)} className="rounded-lg bg-violet-700 text-white px-4 py-1.5 text-xs font-semibold hover:bg-violet-800">
  ...
  return (
    <div className="rounded-xl bg-white border border-neutral-200 p-5 space-y-3">
      ...
      <label className="block text-xs">
        <span className="text-neutral-500">Home market (default currency)</span>
        <select value={home} onChange={(e) => setHome(e.target.value)} className="mt-1 w-56 rounded-lg border border-neutral-300 px-2 py-1.5 text-sm">
          {["MY", "SG", "BN"].map((m) => (
      ...
      <button
        onClick={() => props.onSave({ homeMarket: home, marketsServed: served.length ? served : ["MY"] })}
        className="rounded-lg bg-violet-700 text-white px-4 py-1.5 text-xs font-semibold hover:bg-violet-800"
      >
        {props.saved ? "Saved ✓" : "Save"}
      </button>
    </div>
  ...
  return (
    <div className="rounded-xl bg-white border border-neutral-200 p-5 space-y-3">
      ...
      <div className="flex items-end gap-3">
        <label className="block text-xs">
          <span className="text-neutral-500">Follow up after (hours)</span>
          <input
            type="number"
            min={1}
            max={72}
            ...
            className="mt-1 w-32 rounded-lg border border-neutral-300 px-2 py-1.5 text-sm"
          />
        </label>
        <label className="block text-xs">
          <span className="text-neutral-500">Max follow-ups</span>
          <input
            type="number"
            min={0}
            max={10}
        ...
        <button
          onClick={() => props.onSave({ followUpAfterHours: hours, maxFollowUps: max })}
          className="rounded-lg bg-violet-700 text-white px-4 py-1.5 text-xs font-semibold hover:bg-violet-800"
        >
          {props.saved ? "Saved ✓" : "Save"}
        </button>
    ...
    <div className="rounded-xl bg-white border border-neutral-200 p-5 space-y-4">
      <div>
        ...
        <p className="text-sm text-neutral-500">
          ...
          app's webhooks at <code className="bg-neutral-100 px-1 rounded">/api/webhooks/whatsapp</code> and{" "}
          <code className="bg-neutral-100 px-1 rounded">/api/webhooks/meta</code>. One-click connect (Embedded Signup)
          ...
          {props.channels.map((c) => (
            <li key={c.id} className="flex items-center justify-between rounded-lg bg-neutral-50 px-3 py-2 text-sm">
              <span>
                ...
                <span className="text-neutral-500">{c.displayName || c.externalId}</span>
      ...
      <form onSubmit={connect} className="grid md:grid-cols-2 gap-3">
        <label className="block text-xs">
          ...
          <select value={channel} onChange={(e) => setChannel(e.target.value)} className="mt-1 w-full rounded-lg border border-neutral-300 px-2 py-1.5 text-sm">
        ...
        <label className="block text-xs">
          ...
          <input required value={externalId} onChange={(e) => setExternalId(e.target.value)} className="mt-1 w-full rounded-lg border border-neutral-300 px-2 py-1.5 text-sm" />
        </label>
        <label className="block text-xs">
          <span className="text-neutral-500">Access token (kept server-side, never shown again)</span>
          <input required type="password" value={accessToken} onChange={(e) => setAccessToken(e.target.value)} className="mt-1 w-full rounded-lg border border-neutral-300 px-2 py-1.5 text-sm" />
        </label>
        <label className="block text-xs">
          <span className="text-neutral-500">Display name (optional)</span>
          <input value={displayName} onChange={(e) => setDisplayName(e.target.value)} className="mt-1 w-full rounded-lg border border-neutral-300 px-2 py-1.5 text-sm" />
        </label>
